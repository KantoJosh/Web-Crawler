...
def is_valid(url):
    try:
        ...
        return re.match(r"(.*(\.ics\.uci\.edu\/*|\.cs\.uci\.edu\/|\.informatics\.uci\.edu\/|\.stat\.uci\.edu\/|today\.uci\.edu\/department\/information_computer_sciences).*)",
                        url) and not re.match(r".*replytocom.*|.*pdf.*|.*google.*|/people*|.*grape.ics.uci.edu.*",url) and not re.match(
                ...
                + r"|rm|smil|wmv|swf|wma|zip|rar|gz|c|py|h|cp)$", parsed.path.lower())
